...
/* The generated skeleton describes .rodata/.bss with the kernel BTF typedefs
 * (u64/u32/s32), which don't exist in userspace — provide them. */
...
int main(int argc, char **argv)
{
	...
	while (!exit_req) {
		fprintf(stderr, "scx_kage: normal=%llu penalty=%llu\r",
			(unsigned long long)skel->bss->nr_normal,
			(unsigned long long)skel->bss->nr_penalty);
		sleep(5);
	}
