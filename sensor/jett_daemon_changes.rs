...
fn handle_kernel_event(data: &[u8]) -> i32 {
    ...
    send_to_bifrost(raw_event.pid, raw_event.uid, &comm, raw_event.event_type, &details, verdict);
    0
}
...
fn enforce_quarantine_in_map(pid: u32) {
    ...
    if let Some(fd) = *guard {
        ...
        eprintln!("⚠️  jeTT: quarantine_map fd not available; cannot enforce PID {} in kernel.", pid);
    }
}
